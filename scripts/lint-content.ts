 ...
 * lesson bodies so an unknown `<Explorable id>` — or a `hasCode` flag that
 * contradicts the body — fails too. Runs in CI (`pnpm lint:content`) so a bad
 * manifest, a typo'd frontmatter key, or an unregistered widget id fails a PR.
 * Exits non-zero, naming the offending file, on the first failure.
 *
